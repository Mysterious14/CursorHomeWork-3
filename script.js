...
function deleteDuplicateLetter(message) {
    ...
    let result="";
    for(let i=0;i<message.length;i++){
        message.indexOf(message[i]) != message.lastIndexOf(message[i]) ?   result:result += message[i];
...
document.writeln(`Найбільша цифра в числі ${firstFunctionNamber} = ${getMaxDigit(firstFunctionNamber)}<br><br>`);
...
document.writeln(`Зарплата ${salary} враховуючи податки: ${firstTax}, ${secondTax} = ${salaryWithoutTax(salary, firstTax, secondTax)}<br><br>`);
...
document.writeln(`"${messageToDelete}" без букви '${letterToDelete} -> "${deleteLetters(letterToDelete, messageToDelete)}"<br><br>`);
...
document.writeln(`"${dublicatemessage}" без букв, які повторюються більше 1 разу ->"${deleteDuplicateLetter(dublicatemessage)}"`);